...
```console
nextflow run deng-lab/viroprofiler --input samplesheet.csv -profile singularity
```
...
First, go to the [deng-lab/viroprofiler releases page](https://github.com/deng-lab/viroprofiler/releases) and find the latest version number - numeric only (eg. `1.22.09`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.22.09`.
